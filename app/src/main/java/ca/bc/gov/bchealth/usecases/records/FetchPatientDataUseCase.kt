...
class FetchPatientDataUseCase @Inject constructor(
    ...
    suspend fun execute(
        ...
        if (serviceFeatureFlag.isOrganDonorRegistrationEnabled()) {
            val data =
                patientDataList.firstOrNull { it.type == PatientDataTypeDto.ORGAN_DONOR_REGISTRATION }
            data?.let {
        ...
        if (dataSetFeatureFlag.isDiagnosticImagingEnabled()) {
            val data =
                patientDataList.filter { it.type == PatientDataTypeDto.DIAGNOSTIC_IMAGING_EXAM }
                    .map {
        ...
        if (dataSetFeatureFlag.isBcCancerScreeningEnabled()) {
            val data = patientDataList.filter { it.type == PatientDataTypeDto.BC_CANCER_SCREENING }
                .map {
                    val bcCancerScreeningDataDto = it as BcCancerScreeningDataDto
                    bcCancerScreeningDataDto.patientId = patientId
                    return@map bcCancerScreeningDataDto
                }
            bcCancerScreeningRepository.insert(data)
        }
    }
}
